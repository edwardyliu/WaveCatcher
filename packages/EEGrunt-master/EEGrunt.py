...
class EEGrunt:
    ...
    def load_data(self):
        ...
        if source == 'muse':
            ...
            #raw_data = np.loadtxt(path + filename, delimiter=',', skiprows=skiprows, usecols=cols)
            
        raw_df = pd.read_csv(path + filename)
        raw_df = raw_df.fillna(method='pad')
        raw_df = raw_df.fillna(method='bfill')
        self.raw_data = raw_df.values
        self.t_sec = np.arange(len(self.raw_data)) /self.fs_Hz
    ...
    def spectrogram(self):
        ...
        f_lim_Hz = [1, 50]   # frequency limits for plotting
    ...
    def plot_filename(self,title = ""):
        fn = self.session_title+' '+title+' Channel '+str(self.channel)
        valid_chars = '-_.() abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'
        filename = 'plots/'+(''.join(c for c in fn if c in valid_chars)).replace(' ','_')+'.jpg'
        return filename
